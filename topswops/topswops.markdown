...
Then on each of these decks we can perform a backward search to find the longest size-(_N_+1) deck which could produce it. Assuming those backward searches don't find anything which matches the already-known lower bound for _L(N+1)_, then when doing the search for size _N_+1 we can use _K_ as the bound for our pruning when there is one fixed card at the end of the deck. Since the execution time is proportional to at least _N_! (and in fact more, because larger decks take more moves), it's worth spending a lot more effort at one size to save on the next.

We can find the smallest useful _K_ by taking the best-known deck for size _N+1_ and running it forward until _N+1_ is in the last place. For example, when playing the optimal 10-38 deck, 10 ends up in last place after 18 moves. The remaining 9 cards take 20 moves to complete, thus the smallest useful _K_ is 21; any further than that will not allow us to improve the pruning for size 10.
