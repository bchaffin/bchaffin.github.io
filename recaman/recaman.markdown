...
For disk storage, I built an array of four 8GB hard drives. To maximize speed they were organized as a RAID-0 array, where data is striped across all the drives, giving 4x the read and write bandwidth of a single drive. But that's bad for reliability, because an error on any one drive corrupts the file. To address reliability, I used the [btrfs filesystem](https://en.wikipedia.org/wiki/Btrfs), which does automatic checksumming of all data to detect any corruption. For most of the life of this project I just hoped that everything would run for a long time without crashing or losing power, but eventually this proved too difficult and I had to implement checkpointing, where all the in-memory pages are flushed to disk, and all global data is saved away. This is a time-consuming process, so I didn't automate it, and instead triggered it manually every few weeks. After creating a checkpoint, I used `btrfs snapshot` to create a clone of the whole storage area.
